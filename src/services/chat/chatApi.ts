...
export const chatApi = {
	...
	async getConversations(): Promise<{ data: ChatConversationResponseDTO[] }> {
		...
		console.warn('Unexpected response format from getConversations:', response);
		return { data: [] };
	},
	...
	async getConversation(id: number): Promise<{ data: ChatConversationResponseDTO }> {
		...
		const response = await apiClient.get(`/Chat/conversations/${id}`, token);
		return response.data;
	},
	...
	async getMessages(conversationId: number, page: number = 1, pageSize: number = 50): Promise<{ data: ChatMessageResponseDTO[] }> {
		...
		const response = await apiClient.get(`/Chat/conversations/${conversationId}/messages?${queryParams.toString()}`, token);
		...
		console.warn('Unexpected response format from getMessages:', response);
		return { data: [] };
	},
	...
	async sendVoiceMessage(conversationId: number, file: File, voiceDuration: number): Promise<{ data: ChatMessageResponseDTO }> {
		...
		if (!response.ok) {
			...
			try {
				...
				errorData = { message: errorText || `HTTP ${response.status}: Request failed` };
			}
			throw errorData;
		}
	...
	async markMessagesAsRead(conversationId: number): Promise<void> {
		...
		await apiClient.put(`/Chat/conversations/${conversationId}/read`, undefined, token);
	},
	...
	async getUnreadCount(conversationId: number): Promise<{ data: { UnreadCount: number } }> {
		...
		const response = await apiClient.get(`/Chat/conversations/${conversationId}/unread-count`, token);
		return response.data;
	},
};
